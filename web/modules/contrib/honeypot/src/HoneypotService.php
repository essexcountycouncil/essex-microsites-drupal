...
class HoneypotService implements HoneypotServiceInterface {
  ...
  public function addFormProtection(array &$form, FormStateInterface $form_state, array $options = []): void {
    ...
    if (in_array('time_restriction', $options) && $this->config->get('time_limit') != 0) {
      ...
      if (empty($input['honeypot_time'])) {
        $identifier = $this->honeypotCreateExpirableKey();
      }
      else {
    ...
    if (!empty($options)) {
      $this->moduleHandler->invokeAll('honeypot_add_form_protection', [
        $options,
        $form,
      ]);
    }
  ...
  public function validateTimeRestriction(array &$element, FormStateInterface $form_state, array &$complete_form): void {
    ...
    // Get the time value.
    if ($form_state->getValue('honeypot_time') === 'no_js_available') {
      // Set an error, but do not penalize the user as it might be a legitimate attempt.
      $form_state->setErrorByName('', 'You seem to have Javascript disabled. Please confirm your form submission.');
      $this->log($form_state->getValue('form_id'), 'honeypot_time');
      // Update the value in $form_state and $element.
      $form_state->setValue('honeypot_time', $this->honeypotCreateExpirableKey());
      $element['#value'] = $form_state->getValue('honeypot_time');

      return;
    }

    [$honeypot_time, $identifier] = $this->getHoneyPotTime($element, $form_state);
  ...
  /**
   * Creates an expirable key/value pair for honeypot_time_restriction.
   *
   * @return string
   *   The honeypot_time_restriction key for the key/value pair.
   */
  protected function honeypotCreateExpirableKey(): string {
    $identifier = Crypt::randomBytesBase64();
    $this->keyValue->setWithExpire($identifier, \Drupal::time()->getCurrentTime(), 3600 * 24);

    return $identifier;
  }

  /**
   * Get the value for the honeypot_time form element.
   *
   * When use_js_for_cached_pages is enabled, submissions on cached pages will
   * have a special value for honeypot_time set by Javascript, which will take
   * the form of js_token:[identifier]:[interval]. identifier: the key for the
   * honeypot_time_restriction value from the keyvalue.expirable service
   * interval: the interval, in seconds, between the page load and the form
   * submission
   *
   * @param array $element
   *   The form element.
   ...
   *
   * @return array
   *   An array containing the key/value identifier and honeypot_time.
   */
  protected function getHoneyPotTime(&$element, FormStateInterface $form_state): array {
    if (strpos($form_state->getValue('honeypot_time'), 'js_token:') === 0) {
      $honeypot_time = 0;
      [$token, $interval] = explode('|', $form_state->getValue('honeypot_time'));
      $identifier = str_replace('js_token:', '', $token);
      if ($this->keyValue->has($identifier)) {
        $honeypot_time = $this->timeService->getRequestTime() - $interval;
      }

      $form_state->setValue('honeypot_time', $honeypot_time);
      $element['#value'] = $form_state->getValue('honeypot_time');
    }
    else {
    ...
    return [$honeypot_time, $identifier];
  }
